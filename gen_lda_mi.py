...
def gen_mi(mi, theme, cat):
    ...
    result.set_body(' '.join(body))
    result.set_topics([theme], cat)

    return result


def balance_lda_mi(ddict, all_docs, theme, D, cat, mi):
    """
    ...
    #   result = [inversed_vocab[i] for i in result_index]
        ...
        result = gen_mi(mi, theme, cat)
    return result


def balance_own_lda_mi(ddict, all_docs, theme, D, cat, mi):
    """
